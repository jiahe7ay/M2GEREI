...
def train(args, model, train_features,train_features2, dev_features, tokenizer=None):
        ...
        for epoch in train_iterator:
            ...
            for step, data in enumerate( tqdm(zip(train_dataloader,train_dataloader2))):
                ...
                if (step == 0 and epoch==0) or (step + 1) == len(train_dataloader) - 1:
                    ...
                    dev_score, dev_output, dev_pred, thresh = evaluate(args, model, dev_features, tag="dev",flag="train")
                    print("classifer:")
                    print(dev_output)
    ...
    if args.ablation in ['M2GEREI']:
        ...
        new_layer.extend(['mention_attention'])
        new_layer.extend(["postion_embeding"])
...
def evaluate(args, model, features,tag="dev", features2=None,flag="test"):
    sen_preds = []
    preds, scores, topks = [], [], []
...
def main():
    ...
    if args.load_path != '':
        ...
        train(args, model, train_features,train_features2, dev_features, tokenizer=tokenizer)
