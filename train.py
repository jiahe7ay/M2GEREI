...
def evaluate(args, model, features,tag="dev", features2=None):
    ...
    for batch in dataloader:
        ...
        if args.ablation in ['M2gerri'] and args.evi_eval_mode != 'none':
            inputs['sen_pos'] = batch[6]
...
def main():
    ...
    if True:
        parser.add_argument("--ablation", default="atlop", type=str, choices=['atlop', 'M2gerri'])
    ...
    if 'M2gerri' in args.load_path:
        args.ablation = 'M2gerri'
    ...
    # device_ids = [0, 1]
    if args.load_path != '':
        name = args.load_path.split('/')[1].split('.')[0].split('M2gerri_')[1]
